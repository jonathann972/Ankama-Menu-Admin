package com.ankamagames.berilia.types.graphic
{
   ...
   public class UiRootContainer extends GraphicContainer implements UIComponent
   {
      ...
      public function iAmFinalized(target:FinalizableUIComponent) : void
      {
         ...
         this.render();
         this._ready = true;
         if(Boolean(this.tempHolder) && Boolean(this.tempHolder.parent))
         {
         ...
         if(Boolean(this._properties) && Boolean(this._properties.hasOwnProperty("visible")))
         {
      ...
      public function render() : void
      {
         ...
         if(Boolean(this.uiClass) && Object(this.uiClass).hasOwnProperty("renderUpdate"))
         {
      ...
      public function destroyUi() : void
      {
         ...
         if(this._aFinalizeElements)
         {
            num = int(this._aFinalizeElements.length);
            for(i = 0; i < num; i++)
            {
      ...
      public function makeSnapshot() : void
      {
         ...
         if(Boolean(rg) && Boolean(rg.selectedItem))
         {
      ...
      private function restoreSnapshot() : void
      {
         ...
         if(snapshot)
         {
            if(snapshot.lastHorizontalTabName)
            {
               ...
               if(Boolean(lastTab && lastTab.visible && !lastTab.selected) && Boolean(!lastTab.softDisabled) && !lastTab.disabled)
               {
                  lastTab.selected = true;
                  this.uiClass.onRelease(this.uiClass[snapshot.lastHorizontalTabName]);
               }
               else if(Boolean(this.uiClass) && Boolean("hintsApi" in this.uiClass) && "currentTabName" in this.uiClass)
               {
