package com.ankamagames.berilia.managers
{
   ...
   public class UiModuleManager
   {
      ...
      public function init(filter:Array, filterInclude:Boolean) : void
      {
         ...
         if(this._everyModuleNames == null)
         {
            ...
            if(this._modulesRoot.exists)
            {
               ...
               {
                  if(!(Boolean(file.isDirectory) && !this._uiModulesScripts[file.name]))
                  {
                     if(Boolean(file.isDirectory) && file.name.charAt(0) != ".")
                     {
      ...
      {
         if(this._modules != null && Boolean(this._modules[name]))
         {
            return this._modules[name];
         }
         if(this._disabledModules != null && Boolean(this._disabledModules[name]))
         {
      ...
      public function unloadModule(id:String) : void
      {
         ...
         if(Boolean(m.mainClass) && m.mainClass.hasOwnProperty("unload"))
         {
            m.mainClass["unload"]();
         }
      ...
      private function searchDmOrD2uiFile(rootPath:File) : File
      {
         ...
         {
            if(!file.isDirectory && Boolean(file.extension))
            {
