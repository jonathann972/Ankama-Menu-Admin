...
namespace Giny.IO.D2O
{
    public class D2OWriter : IDisposable
    {
        ...
        /// <summary>
        /// Serializes a single object with the class definitions loaded from the
        /// source D2O without rebuilding the complete file.
        /// </summary>
        ...
        /// <summary>
        /// Replaces or adds one indexed object while preserving the original
        /// object area, class definitions and search table byte for byte.
        /// </summary>
        public static void UpsertObjectPreservingFile(string filename, int index, object obj)
        {
            int indexTableOffset;
            using (var source = File.OpenRead(filename))
            {
                var reader = new D2OReader(source);
                indexTableOffset = reader.IndexTableOffset;
                reader.Close();
            }

            byte[] serialized;
            using (var serializer = new D2OWriter(filename))
                serialized = serializer.SerializeObject(obj);
            ...
            if (entryOffset >= 0)
            {
                ...
                BinaryPrimitives.WriteInt32BigEndian(original.AsSpan(entryOffset + 4, 4), objectOffset);
            ...
            var appendedObjectOffset = original.Length + 8;
            ...
            BinaryPrimitives.WriteInt32BigEndian(result.AsSpan(insertionOffset + 4, 4), appendedObjectOffset);
        ...
        public static void AddIndexesToIntSearchValue(string filename, string fieldName, int value, params int[] indexesToAdd)
        {
            D2OSearchEntry entry;
            int searchDataOffset;
            using (var source = File.OpenRead(filename))
            {
                var reader = new D2OReader(source);
                entry = reader.SearchEntries[fieldName];
                searchDataOffset = reader.SearchDataOffset;
                reader.Close();
            }
            if (entry.FieldType != D2OFieldType.Int && entry.FieldType != D2OFieldType.UInt && entry.FieldType != D2OFieldType.I18N)
                throw new InvalidOperationException($"Search field '{fieldName}' is not an integer field.");

            var original = File.ReadAllBytes(filename);
            var position = entry.FieldIndex;
            using var rebuilt = new MemoryStream();
            var found = false;
            for (var group = 0; group < entry.FieldCount; group++)
            {
                var groupValue = BinaryPrimitives.ReadInt32BigEndian(original.AsSpan(position, 4));
                rebuilt.Write(original, position, 4);
                position += 4;
                var byteLength = BinaryPrimitives.ReadInt32BigEndian(original.AsSpan(position, 4));
                position += 4;
                var existingIndexes = new List<int>();
                for (var offset = 0; offset < byteLength; offset += 4)
                    existingIndexes.Add(BinaryPrimitives.ReadInt32BigEndian(original.AsSpan(position + offset, 4)));
                existingIndexes.RemoveAll(indexesToAdd.Contains);
                if (groupValue == value)
                {
                    existingIndexes.AddRange(indexesToAdd.Distinct());
                    found = true;
                }
                Span<byte> lengthBytes = stackalloc byte[4];
                BinaryPrimitives.WriteInt32BigEndian(lengthBytes, existingIndexes.Count * 4);
            ...
            if (!found)
            {
                ...
                Span<byte> lengthBytes = stackalloc byte[4];
                BinaryPrimitives.WriteInt32BigEndian(lengthBytes, indexesToAdd.Distinct().Count() * 4);
                rebuilt.Write(lengthBytes);
                foreach (var index in indexesToAdd.Distinct())
                {
                    Span<byte> indexBytes = stackalloc byte[4];
                    BinaryPrimitives.WriteInt32BigEndian(indexBytes, index);
                    rebuilt.Write(indexBytes);
                }
                BinaryPrimitives.WriteInt32BigEndian(original.AsSpan(entry.DescriptorFieldCountPosition, 4), entry.FieldCount + 1);
            }

            var newBlockOffset = original.Length;
            BinaryPrimitives.WriteInt32BigEndian(original.AsSpan(entry.DescriptorOffsetPosition, 4), newBlockOffset - searchDataOffset);
        ...
        public static void AddIndexesToBoolSearchValue(string filename, string fieldName, bool value, params int[] indexesToAdd)
        {
            D2OSearchEntry entry;
            int searchDataOffset;
            using (var source = File.OpenRead(filename))
            ...
            if (entry.FieldType != D2OFieldType.Bool)
                throw new InvalidOperationException($"Search field '{fieldName}' is not a boolean field.");
            ...
            for (var group = 0; group < entry.FieldCount; group++)
            {
                var groupValue = original[position++] != 0;
                rebuilt.WriteByte(groupValue ? (byte)1 : (byte)0);
                var byteLength = BinaryPrimitives.ReadInt32BigEndian(original.AsSpan(position, 4));
                position += 4;
                var existingIndexes = new List<int>();
                for (var offset = 0; offset < byteLength; offset += 4)
            ...
            if (!found)
            {
                rebuilt.WriteByte(value ? (byte)1 : (byte)0);
                Span<byte> lengthBytes = stackalloc byte[4];
                var distinctIndexes = indexesToAdd.Distinct().ToArray();
                BinaryPrimitives.WriteInt32BigEndian(lengthBytes, distinctIndexes.Length * 4);
                rebuilt.Write(lengthBytes);
        ...
        private void WriteField(IDataWriter writer, D2OFieldType fieldType, D2OFieldDefinition field, dynamic obj, int vectorDimension = 0)
        {
            switch (fieldType)
            {
                case D2OFieldType.Int:
                    // Les D2O stockent parfois un uint dans un champ AS3 "int".
                    // Conserver les 32 bits bruts au lieu de lever une OverflowException.
                    WriteFieldInt(writer, obj is uint ? unchecked((int)obj) : Convert.ToInt32(obj));
                    break;
