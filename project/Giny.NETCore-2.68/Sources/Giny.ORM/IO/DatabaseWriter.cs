...
namespace Giny.ORM.IO
{
    public class DatabaseWriter
    {
        ...
        private void AddElements(IRecord[] elements)
        {
            ...
            string columns = string.Join(",", AddProperties.Select(property => $"`{property.Name}`"));
            command.CommandText = string.Format(QueryConstants.Insert, TableName, columns, string.Join(",", final));
