...
namespace Giny.World.Managers.Exchanges.Jobs
{
    ...
    {
        private const double DefaultFirstBreakBonus = 40d; // 4000%
        ...
        private List<CharacterItemRecord> DecraftItem(AsyncRandom random, CharacterItemRecord item)
        {
            ...
            {
                var runes = ItemsManager.Instance.GetRunesItem(effect.EffectEnum).ToArray();

                foreach (var rune in runes)
                {
                    var runeEffect = rune.Effects.GetFirst<EffectInteger>(effect.EffectEnum);

                    if (runeEffect != null && runeEffect.Value > 0)
                    {
                        var itemEffectValue = Math.Max(1, Math.Abs(effect.Value));
                        var theoreticalQuantity = itemEffectValue / (double)runeEffect.Value;
                        var quantity = Math.Max(1, (int)Math.Floor(theoreticalQuantity * DefaultFirstBreakBonus));
