...
namespace Giny.World.Managers.Items.Collections
{
    public class Inventory : ItemCollection<CharacterItemRecord>
    {
        ...
        public CharacterItemRecord AddItem(int gid, int quantity, bool perfect = false)
        {
            ...
            if (template != null)
            {
                if (AnomalyRollManager.Instance.HasDefinition(gid) && quantity > 1)
                {
                    CharacterItemRecord lastCreated = null;
                    for (var i = 0; i < quantity; i++)
                    {
                        lastCreated = ItemsManager.Instance.CreateCharacterItem(template, Character.Id, 1, perfect);
                        base.AddItem(lastCreated);
                    }
                    return lastCreated;
                }
        ...
        {
            if (AnomalyRollManager.Instance.HasDefinition(gid))
                return null;
        ...
        private void UpdateLook(CharacterItemRecord item, bool equiped)
        {
            switch (item.Record.TypeEnum)
            {
                ...
                case ItemTypeEnum.PETSMOUNT:
                    ...
                    break;
                default:
                    // === DOFUS ITEM STUDIO V3 APPEARANCE FIX ===
                    // === DOFUS ITEM STUDIO V3.1 VISUAL TEMPLATE-FIRST ===
                    // Le template importé est la source de vérité.
                    short v31AppearanceId =
                        (item.Record != null && item.Record.AppearenceId != 0)
                        ? item.Record.AppearenceId
                        : item.AppearanceId;

                    if (v31AppearanceId != 0)
                    {
                        // Synchronise aussi l'instance persistée character_items.
                        if (item.AppearanceId != v31AppearanceId)
                        {
                            item.AppearanceId = v31AppearanceId;
                            item.UpdateLater();
                        }

                        if (equiped)
                            Character.Look.AddSkin(v31AppearanceId);
                        else
                            Character.Look.RemoveSkin(v31AppearanceId);
                    }
                    break;
        ...
        private void SetItemPosition(CharacterItemRecord item, CharacterInventoryPositionEnum position, int quantity)
        {
            if (item == null)
            {
                Character.ReplyWarning("[EQUIPDEBUG] item=NULL | requestedPosition=" + position);
                return;
            }

            // === DOFUS ITEM STUDIO V7.0.3 EQUIP TRACE ===
            Character.Reply("[EQUIPDEBUG] REQUEST GID=" + item.GId +
                " UID=" + item.UId +
                " currentPosition=" + item.Position +
                " requestedPosition=" + (byte)position +
                " type=" + item.Record.TypeId +
                " level=" + item.Record.Level +
                " charLevel=" + Character.Level +
                " criteria=" + (string.IsNullOrEmpty(item.Record.Criteria) ? "<empty>" : item.Record.Criteria) +
                " hasSet=" + item.Record.HasSet);

            if (item.Record.TypeEnum == ItemTypeEnum.LIVING_OBJECT || item.Record.IsCeremonialItem())
            {
                if (Character.Fighting)
                {
                    Character.ReplyWarning("[EQUIPDEBUG] REFUS=FIGHTING_LIVING_OR_CEREMONIAL");
                    return;
                }
            }
            ...
            if (position != CharacterInventoryPositionEnum.INVENTORY_POSITION_NOT_EQUIPED)
            {
                ...
                {
                    Character.ReplyWarning("[EQUIPDEBUG] REFUS=LEVEL_TOO_LOW");
                    OnError(ObjectErrorEnum.LEVEL_TOO_LOW);
                    return;
                }
                Character.Reply("[EQUIPDEBUG] levelCheck=OK");
                ...
                Character.Reply("[EQUIPDEBUG] criteriaCheck=" + (result ? "OK" : "FAIL"));
                if (!result)
                {
                    Character.ReplyWarning("[EQUIPDEBUG] REFUS=CRITERIA");
                ...
                {
                    Character.ReplyWarning("[EQUIPDEBUG] REFUS=DOFUS_POSITION_GUARD");
                    return;
                }

                if (CheckStacks(item, position, RingPositions) && item.Record.HasSet)
                {
                    Character.ReplyWarning("[EQUIPDEBUG] REFUS=RING_STACK");
                ...
                {
                    Character.ReplyWarning("[EQUIPDEBUG] REFUS=DOFUS_STACK");
                    OnError(ObjectErrorEnum.CANNOT_EQUIP_HERE);
                    return;
                }

                bool equipEventOk = InventoryEventApi.OnItemEquipping(Character, item);
                Character.Reply("[EQUIPDEBUG] InventoryEventApi.OnItemEquipping=" + (equipEventOk ? "OK" : "FAIL"));
                if (!equipEventOk)
                {
                    Character.ReplyWarning("[EQUIPDEBUG] REFUS=INVENTORY_EVENT");
                    return;
                }
                ...
                Character.Reply("[EQUIPDEBUG] ACCEPT -> EquipItem | old=" + item.Position + " requested=" + (byte)position);
                EquipItem(item, position, quantity);
                Character.Reply("[EQUIPDEBUG] AFTER EquipItem | nowPosition=" + item.Position);
            }
            else
            {
                if (item.PositionEnum == CharacterInventoryPositionEnum.INVENTORY_POSITION_NOT_EQUIPED)
