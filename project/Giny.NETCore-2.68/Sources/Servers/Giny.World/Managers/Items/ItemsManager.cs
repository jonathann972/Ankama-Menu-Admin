...
namespace Giny.World.Managers.Items
{
    public class ItemsManager : Singleton<ItemsManager>
    {
        ...
        public void Reload()
        {
            // === DOFUS ITEM STUDIO ITEMSET RELOAD ===
            // Dependencies must exist and be initialized before ItemRecord.Initialize.
            DatabaseManager.Instance.Reload<ItemSetRecord>();
            ItemSetRecord.Initialize();
        ...
        {
            var effects = record.Effects.Generate(perfect);
            AnomalyRollManager.Instance.AddGeneratedRolls((int)record.Id, effects);

            return new CharacterItemRecord(characterId, 0, (short)record.Id, (byte)CharacterInventoryPositionEnum.INVENTORY_POSITION_NOT_EQUIPED, quantity, effects,
